...
function dataHora(data: string) {
  ...
  return {
    data: new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "short" }).format(valor),
    hora: new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(valor),
  }
}

export function UpcomingAppointments({ consultas }: { consultas: ProximaConsulta[] }) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="flex flex-col gap-2">
        {consultas.length === 0 && (
          <p className="py-10 text-center text-sm text-muted-foreground">Nenhuma consulta agendada.</p>
        )}
          ...
          return (
            ...
            >
              <div className="flex size-14 shrink-0 flex-col items-center justify-center rounded-lg bg-primary/10 text-primary">
                <span className="text-[10px] font-medium uppercase">{horario.data}</span>
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold">{consulta.pacienteNome}</p>
                <p className="truncate text-xs text-muted-foreground">{tipoLabels[consulta.tipo]}</p>
              </div>
              <span className={cn("shrink-0 rounded-full border px-2.5 py-1 text-xs font-medium", statusStyles[consulta.status])}>
