...
function paraApi(data: string) {
  const [a, m, d] = data.split("-");
  return `${d}/${m}/${a}`;
}
function paraInput(data: string) {
...
export default function Pacientes() {
  ...
  const filtrados = useMemo(
    () =>
      pacientes.filter((p) =>
        `${p.nome} ${p.email}`.toLowerCase().includes(busca.toLowerCase()),
      ),
    [pacientes, busca],
  );
  function abrirNovo() {
  ...
  function abrirEdicao(p: Paciente) {
    ...
    setForm({
      nome: p.nome,
      dataNascimento: paraInput(p.dataNascimento),
      sexo: p.sexo,
      email: p.email,
      telefone: p.telefone ?? "",
  ...
  async function alternar(p: Paciente) {
    try {
      await atualizarStatusPaciente(p.id, !p.ativo);
      await carregar();
    } catch (e) {
      setErro(mensagemDaApi(e));
    }
  ...
  return (
    <main className="flex-1 space-y-6 p-5 sm:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="font-serif text-2xl font-bold">Pacientes</h1>
              ...
              {carregando ? (
                ...
                filtrados.map((p) => (
                  <tr key={p.id} className="border-b last:border-0">
                    ...
                    <td className="p-4">{p.dataNascimento}</td>
                    <td className="p-4">
                      <button
                        onClick={() => void alternar(p)}
                        className={`rounded-full px-2.5 py-1 text-xs font-medium ${p.ativo ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                      >
